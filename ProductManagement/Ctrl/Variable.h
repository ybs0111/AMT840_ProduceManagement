...
{
	int					nUph[24];				// 시간당 생산량.....
	int					nSocketOff[24];			// 시간당 Socket Off......
	int					nCok[24];				// 시간당 Cok 교체.....
	double				dRunTime[24];			// 시간당 가동 시간.......
	double				dStopTime[24];			// 시간당 stop 시간......
	double				dIdleTime[2][24];		// 시간당 idle 시간.....
	double				dAlarmTime[24];			// 시간당 alarm 시간.....
	int					nAlarmCount[24];		// 시간당 alarm 수량....
	int					nRetest[24];			// 시간당 retest 발생횟수.....
	double				dMtbi[24];				// 시간당 mtbi.....
	
	int					nPartNo;
	CString				strPartNo[100];		// 시간당 cycle time.....
	
	double				dCycleHour[24];
	int					nCycleHour[24];
	double				dCyclePart[24][100];	// 시간당 cycle time......
	int					nCyclePart[24][100];
	double				dCyclePartTot[100];
	int					nCyclePartTot[100];
	double				dCycleAve[24][100];
	int					nCycle[24][100];

	double				dTestHour[24];
	int					nTestHour[24];
	double				dTestPart[24][100];	// 시간당 cycle time......
	int					nTestPart[24][100];
	double				dTestPartTot[100];
	int					nTestPartTot[100];
	double				dTestAve[24][100];
	int					nTest[24][100];
